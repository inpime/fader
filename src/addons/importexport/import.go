...
func AppImportFromLastArchive() error {
	...
	logrus.WithField("_service", addonName).Infof("Downloading on link %q...", ArchiveURLLatestVersion)

	data, err := loadLatestArchive(ArchiveURLLatestVersion)

	if err != nil {
		logrus.WithField("_service", addonName).Errorf("Error download, more details: %q", err)
		return err
	}

	if err := makeImportImportExport(data); err != nil {
		logrus.WithField("_service", addonName).WithError(err).Errorf("Error importing.")
		return err
	}

	logrus.WithField("_service", addonName).Debug("OK importing.")
